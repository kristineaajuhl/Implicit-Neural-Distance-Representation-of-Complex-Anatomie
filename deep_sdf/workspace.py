...
classifier_params_subdir = "ClassifierParameters"
...
def load_classifier_parameters(experiment_directory, checkpoint, classifier):

    filename = os.path.join(
        experiment_directory, classifier_params_subdir, checkpoint + ".pth"
    )

    if not os.path.isfile(filename):
        raise Exception('classifier state dict "{}" does not exist'.format(filename))
